...
def squad_f1(preds, labels):
    ...
    for i in range(n_docs):
        ...
        for label in labels[i]:
            pre, rec, b_f1 = squad_f1_single(best_pred, label)
    ...
    mean_pre = float(np.mean(precisions))
...
def squad_base(preds, labels):
    ...
    precision,recall,f1 = squad_f1(preds=preds, labels=labels)
    top_acc = top_n_accuracy(preds=preds, labels=labels)
    return {"EM": em,"Precision": precision,"Recall": recall, "f1": f1, "top_n_accuracy": top_acc}
...
def squad(preds, labels):
    ...
    return {"EM": overall_results["EM"],"Precision": overall_results["Precision"],"Recall": overall_results["Recall"], "f1": overall_results["f1"], "top_n_accuracy": overall_results["top_n_accuracy"],
...
def top_n_accuracy(preds, labels):
    ...
    for i in range(n_questions):
        ...
        for idx, pred in enumerate(current_preds):
            precision,recall,f1_score = max([squad_f1_single(current_preds, label, pred_idx=idx) for label in labels[i]])
            if f1_score:
                break
        if f1_score:

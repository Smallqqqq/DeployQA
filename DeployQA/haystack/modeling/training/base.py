...
class Trainer:
    ...
    def train(self):
        ...
        tb_writer = SummaryWriter(self.log_root_dir)
            ...
            for step, batch in enumerate(progress_bar):
                ...
                if self.evaluate_every != 0 \
                    ...
                    if dev_data_loader is not None:
                        ...
                        tb_writer.add_scalar('f1 score',result[0]["f1"],self.global_step)
                        tb_writer.add_scalar('precision', result[0]["Precision"], self.global_step)
                        tb_writer.add_scalar('recall', result[0]["Recall"], self.global_step)
                        #print(result)
                ...
                #print("print global step",self.global_step)
                #print("print check point_every and loss",self.checkpoint_every,loss)
                if self.log_every and self.global_step % self.log_every == 0:
                    tb_writer.add_scalar('loss',loss,self.global_step)

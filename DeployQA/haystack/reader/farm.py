...
class FARMReader(BaseReader):
    """
    Transformer based model for extractive Question Answering using the f (https://github.com/deepset-ai/FARM).
    ...
    def eval_on_file(self, data_dir: str, test_filename: str, device: Optional[str] = None):
        ...
        results = {
            ...
            "Precision": eval_results[0]["Precision"],
            "Recall": eval_results[0]["Recall"],
            "f1": eval_results[0]["f1"],
            "top_n_accuracy": eval_results[0]["top_n_accuracy"],
            "labels":eval_results[0]["labels"],
                ...
                for label in aggregated_per_doc[doc_id]:
                    ...
                    if id_question_tuple in aggregated_per_question.keys():
                        if label.offset_start_in_doc == 0 and label.answer == "":
                            ...
                            if len(aggregated_per_question[id_question_tuple]["answers"]) >= 100:

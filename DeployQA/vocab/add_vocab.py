...
idf_sorted_indexes = sorted(range(len(idf)), key=lambda k: idf[k] * -1)
idf_sorted = idf[idf_sorted_indexes]  # token and its index number's idf value
tokens_by_df = np.array(vectorizer.get_feature_names())[idf_sorted_indexes]
...
new_tokens = tokens_by_df[0:11400]
